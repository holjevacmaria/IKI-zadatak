...
const buttonLoadMore = document.querySelector('.button-loadmore');
const navigacija = document.getElementById("navigation");
const overlayContent = document.querySelector('.overlayContent');
const element = document.querySelector('.button-loadmore');
const buttonMovieRoulette = document.querySelector('.button-roulette');

/*****rendera filmove u li-jeve*****/

const renderMovie = (movie, i) => {
    const markup = `
    <li class="movie-box movie" data-itemid=${i}>
        <div class="movie-img"><img src ="https://image.tmdb.org/t/p/w185/${movie.poster_path}" alt="${movie.title}"></div>
        <div class="movie-name">${limitMovieTitle(movie.title)} (${limitMovieYear(movie.release_date)})</div>
...
const renderAMovie = (movie, i) => {
    ...
    <li class="movie-ol"  data-itemid=${i}>
        <div class="movie-overlay-row">
            <div class="movie-overlay-name">${movie.title} (${limitMovieYear(movie.release_date)})</div>
            <a class="closebtn" onclick="clearOverlay()">&times;</a>
        </div>
        <div class="movie-overlay-img" style = background-image:url("https://image.tmdb.org/t/p/w780/${movie.backdrop_path}"); alt="${movie.title}">
            <div class='rating movie-star-rating' data-itemid=${movie.id}>
                ${stars}
            </div>    
        </div>
...
renderer.addEventListener('click', e => {
    const id = e.target.closest('.movie-box').dataset.itemid;
...
buttonMovieRoulette.addEventListener('click', e => {
    e.preventDefault();
    navigacija.style.display = "block";
    const miniMarkup = `<div class="form-row"><a class="form-exit" onclick="clearOverlay()">&times;</a></div><div class="genres-form"></div>`;
    overlayContent.insertAdjacentHTML('beforeend', miniMarkup);
    renderGenres(movieGenresState.search.genre.data.genres);
    const buttonMarkup = `<button class="genres-roll">Roll</button>`;
    overlayContent.insertAdjacentHTML('beforeend', buttonMarkup);
    document.querySelector('.form-input').checked = true;
});
/*****rendera žanrove*****/
...
const renderGenre = (genre) => {
    ...
    <input type="radio" class="form-input" name="genre-type" value=${genre.id} id="${genre.name}">
    <label for="${genre.name}" class="radio-label">
...
overlayContent.addEventListener('click', e => {
    let genreId = 0;
    if (e.target.matches('.genres-roll')) {
...
/*****brisanje childova diva s klasom "overlayContent"*****/
...
const generateOneMovieByGenre = (rouletteMovie) => {
    ...
    <li>
        <div class="movie-overlay-row">
            <div class="movie-overlay-name">${rouletteMovie[0].title} (${limitMovieYear(rouletteMovie[0].release_date)})</div>
            <a class="closebtn" onclick="clearOverlay()">&times;</a>
        </div>
        <div class="movie-overlay-img" style=background-image:url("https://image.tmdb.org/t/p/w780/${rouletteMovie[0].backdrop_path}") alt="${rouletteMovie[0].title}">
            <div class='rating movie-star-rating' data-itemid=${rouletteMovie[0].id}>
                ${stars}
            </div>         
	    </div>
